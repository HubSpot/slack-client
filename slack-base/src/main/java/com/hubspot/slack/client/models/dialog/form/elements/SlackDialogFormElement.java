  ...
  protected void validateBaseElementProperties(SlackDialogFormElement normalized) {
    ...
    if (normalizedLabel.length() > maxLabelLength) {
      String errorMessage = String.format(
        "Label cannot exceed %s chars, got %s",
        maxLabelLength,
        normalizedLabel
      );
    ...
    if (normalizedName.length() > maxNameLength) {
      String errorMessage = String.format(
        "Name cannot exceed %s chars, got %s",
        maxNameLength,
        normalizedName
      );
      throw new IllegalStateException(errorMessage);
    ...
    int maxPlaceholderLength =
      SlackDialogFormElementLengthLimits.MAX_PLACEHOLDER_LENGTH.getLimit();
    if (
      normalizedPlaceholder.isPresent() &&
      normalizedPlaceholder.get().length() > maxPlaceholderLength
    ) {
      String errorMessage = String.format(
