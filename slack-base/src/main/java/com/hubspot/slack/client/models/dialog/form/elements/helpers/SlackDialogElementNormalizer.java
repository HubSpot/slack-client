...
public class SlackDialogElementNormalizer {
  ...
  public static AbstractSlackFormTextElement normalize(
    AbstractSlackFormTextElement element
  ) {
    if (
      shouldNormalizePlaceholder(element) ||
      shouldNormalizeLabel(
        element,
        SlackDialogFormElementLengthLimits.MAX_LABEL_LENGTH
      ) ||
      shouldNormalize(
        element.getHint(),
        SlackDialogFormElementLengthLimits.MAX_HINT_LENGTH
      )
    ) {
      return SlackFormTextElement
        .builder()
        .from(element)
  ...
  public static AbstractSlackFormTextareaElement normalize(
    AbstractSlackFormTextareaElement element
  ) {
    if (
      shouldNormalizePlaceholder(element) ||
      shouldNormalizeLabel(
  ...
  public static AbstractSlackFormSelectElement normalize(
    AbstractSlackFormSelectElement element
  ) {
    if (
      shouldNormalizePlaceholder(element) ||
      shouldNormalizeLabel(
      ...
      shouldNormalize(
        element.getOptionGroups(),
        SlackDialogFormElementLengthLimits.MAX_OPTION_GROUPS_NUMBER
      ) ||
      shouldNormalizeOptions(element)
    ) {
      return SlackFormSelectElement
        .builder()
        .from(element)
        .setPlaceholder(normalizePlaceholder(element))
        .setLabel(normalizeLabel(element))
  ...
  public static SlackFormOptionGroupIF normalize(SlackFormOptionGroupIF element) {
    if (
      shouldNormalizeLabel(
        element,
        SlackDialogFormElementLengthLimits.MAX_OPTION_LABEL_LENGTH
      ) ||
      shouldNormalizeOptions(element)
    ) {
  ...
  private static boolean shouldNormalizeOptions(HasOptions element) {
    return shouldNormalize(
      element.getOptions(),
      SlackDialogFormElementLengthLimits.MAX_OPTIONS_NUMBER
    );
  }
  ...
  private static boolean shouldNormalize(
    ...
    SlackDialogFormElementLengthLimits maxListSize
  ) {
  ...
  private static boolean shouldNormalizePlaceholder(SlackDialogFormElement element) {
    return shouldNormalize(
      ...
      SlackDialogFormElementLengthLimits.MAX_PLACEHOLDER_LENGTH
    );
  }

  private static boolean shouldNormalizeLabel(
    HasLabel element,
    SlackDialogFormElementLengthLimits maxLabelLength
  ) {
  ...
  private static boolean shouldNormalize(
    ...
    return (
      ...
      shouldNormalize(stringOptional.get(), maxPlaceholderLength)
    );
  }
  ...
  private static String normalizeLabel(SlackFormOptionIF option) {
    return normalizeIfLongerThan(
      option.getLabel(),
      SlackDialogFormElementLengthLimits.MAX_OPTION_LABEL_LENGTH.getLimit()
    );
  }
  ...
  private static Iterable<SlackFormOptionGroup> normalizeOptionGroups(
    AbstractSlackFormSelectElement element
  ) {
  ...
  private static Iterable<SlackFormOption> normalizeOptions(
    AbstractSlackFormSelectElement element
  ) {
    return Iterables.limit(
      element.getOptions(),
  ...
  private static Iterable<SlackFormOption> normalizeOptions(
    SlackFormOptionGroupIF element
  ) {
    return Iterables.limit(
      element.getOptions(),
      SlackDialogFormElementLengthLimits.MAX_OPTIONS_NUMBER.getLimit()
  ...
  private static String normalizeLabel(SlackDialogFormElement element) {
    return normalizeIfLongerThan(
      element.getLabel(),
      SlackDialogFormElementLengthLimits.MAX_LABEL_LENGTH.getLimit()
    );
  }
  ...
  private static Optional<String> normalizeHint(
    AbstractSlackDialogFormTextElement element
  ) {
    return element
      .getHint()
      .map(hint ->
        normalizeIfLongerThan(
          hint,
          SlackDialogFormElementLengthLimits.MAX_HINT_LENGTH.getLimit()
        )
      );
  }
