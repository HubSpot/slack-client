...
public class ViewSubmissionDeserializationTest {
  ...
  public static void setUp() throws IOException {
    ...
    final ViewSubmission viewSubmission = ObjectMapperUtils
      .mapper()
      .readValue(raw, ViewSubmission.class);
    BLOCK_ID_TO_ACTION_ID_TO_VALUES =
      viewSubmission.getView().getState().getBlockIdToActionIdToValues();
  }
  ...
  public void itDeserializesCheckboxes() {
    ...
    assertThat(checkboxes.getSelectedOptions().get(0).getText().getText())
      .isEqualTo("The checkbox label");
    assertThat(checkboxes.getSelectedOptions().get(0).getDescription().isPresent())
      .isEqualTo(false);
  ...
  public void itDeserializesMultiStaticSelect() {
    ViewInput input = BLOCK_ID_TO_ACTION_ID_TO_VALUES
      .get("YjQ")
      .get("multi_static_select-action");
    ...
    Option anotherOption = TestBlocksBuilder.buildOption(
      anotherOptionValue,
      anotherOptionText
    );

    final ViewMultiStaticSelect multiStaticSelect = (ViewMultiStaticSelect) input;
    assertThat(multiStaticSelect.getSelectedOptions())
  ...
  public void itDeserializesMultiExternalSelect() {
    ViewInput input = BLOCK_ID_TO_ACTION_ID_TO_VALUES
      .get("BjF")
      .get("multi_external_select-action");
