...
class ReflectionBasedFieldPresenceTest {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(
    ReflectionBasedFieldPresenceTest.class
  );

  private static final Number DEFAULT_NUMBER = 5;
  private static final Boolean DEFAULT_BOOLEAN = false;
  private static final String DEFAULT_STRING = "default";
  private static final Optional<String> DEFAULT_OPTIONAL_STRING = Optional.of(
    "defaultOptional"
  );
  private static final Set<String> OPTIONAL_STRING_METHODS_TO_BUILD =
    Collections.singleton("setText");
  private static final Map DEFAULT_MAP = Collections.emptyMap();

  private static final Reflections SLACK_CLIENT_REFLECTOR = new Reflections(
    "com.hubspot.slack.client"
  );

  static <T> Set<Class<? extends T>> getConcreteFinalSubclasses(Class<T> baseClass) {
    return SLACK_CLIENT_REFLECTOR
  ...
  static <T> Object buildTestInstance(Class<? extends T> base) throws Exception {
    Optional<Method> builderFactoryMethodMaybe = ReflectionUtils
      .getMethods(base, ReflectionUtils.withName("builder"))
      .stream()
    ...
    for (Method builderMethod : builderMethods) {
      ...
      if (isNumeric(singletonParamType)) {
        ...
        LOG.debug(
          "Calling {} with default optional of {}",
          builderMethod.getName(),
          DEFAULT_OPTIONAL_STRING
        );
        builderMethod.invoke(builder, DEFAULT_OPTIONAL_STRING);
      } else if (
        Boolean.class.isAssignableFrom(singletonParamType) ||
        boolean.class.isAssignableFrom(singletonParamType)
      ) {
        LOG.debug(
          "Calling {} with default of {}",
        ...
        LOG.debug(
          "Recursively constructing test instance for {}",
          builderMethod.getName()
        );
    ...
    Optional<Method> buildMethodMaybe = ReflectionUtils
      .getMethods(builder.getClass(), ReflectionUtils.withName("build"))
      .stream()
      .findFirst();
    if (!buildMethodMaybe.isPresent()) {
      throw new IllegalStateException(
        String.format("Couldn't find builder 'build' method on %s", builder.getClass())
      );
    }
  ...
  static void verifyHasField(Class<?> base, Object object, String field)
    throws Exception {
